 ...
 * mw.loader.load('//www.wikidata.org/w/index.php?title=User:Danielyepezgarces/Gadget-cradle.js&action=raw&ctype=text/javascript&version=1.6.0');
 */
...
(function() {
    ...
    let isCradleReport = (pageName === 'Special:CradleReport' || pageName === 'Special:BlankPage/CradleReport');
    
    // Inject early hide CSS rules to prevent any default content from flashing before script loads
    if (isSpecialCradle || isCradleReport) {
        let style = document.createElement('style');
        style.id = 'cradle-early-hide-style';
        style.innerHTML = '#mw-content-text > *:not(.cradle-fullpage-container):not(.cradle-report-container) { display: none !important; } #firstHeading { visibility: hidden !important; }';
    ...
    function proceedInit() {
        ...
        mw.util.addPortletLink(
            'p-tb',
            mw.util.getUrl('Special:CradleReport'),
            'Cradle Report (Beta)',
            't-cradle-report',
            'Generate data quality reports based on schemas'
        );

        if (isSpecialCradle) {
            setupSpecialPage();
        } else if (isCradleReport) {
            setupReportPage();
        } else if (isItemPage) {
            createHeadingButton();
    ...
    /**
     * Set up Special:CradleReport full page view.
     */
    function setupReportPage() {
        document.title = "Cradle Report - Wikidata Data Quality";
        
        let $heading = $('#firstHeading');
        if ($heading.length) {
            $heading.text('Cradle Report (Beta)');
        ...
        let $container = $('<div>').addClass('cradle-report-container').css({
            'max-width': '1000px',
            'margin': '0 auto',
            'font-family': 'sans-serif',
            'color': 'var(--color-base, #202122)',
        ...
        // Add CSS styling inside the report
        let reportCSS = `
            .cradle-report-card {
                background: #ffffff;
                border: 1px solid var(--border-color-base, #a2a9b1);
            ...
            .cradle-report-table th, .cradle-report-table td {
                border: 1px solid var(--border-color-base, #a2a9b1);
                padding: 10px;
                text-align: left;
            }
            .cradle-report-table th {
                background-color: var(--background-color-neutral-subtle, #f8f9fa);
                font-weight: bold;
            }
            .cradle-report-progress {
                width: 100%;
        ...
        `;
        mw.util.addCSS(reportCSS);

        let $selectorDiv = $('<div>').addClass('cradle-report-card');
        $selectorDiv.append($('<h3>').text('Configure Schema Report'));
        $selectorDiv.append($('<p>').text('Validate a sample of Wikidata items against a schema or template.'));

        // Input 1: EntitySchema ID
        let $schemaInputRow = $('<div>').css({'margin-bottom': '16px'});
        $schemaInputRow.append($('<label>').text('EntitySchema ID (e.g. E10): ').css({'display': 'block', 'font-weight': 'bold', 'margin-bottom': '6px'}));
        let $schemaInput = $('<input>')
            .attr('type', 'text')
            .attr('placeholder', 'Enter ID (e.g. E10)')
            .addClass('cradle-input')
            .css({'max-width': '300px'});
        $schemaInputRow.append($schemaInput);
        $selectorDiv.append($schemaInputRow);

        // Input 2: Class QID (Optional if schema has links, but useful if schema is not linked)
        let $classInputRow = $('<div>').css({'margin-bottom': '16px'});
        $classInputRow.append($('<label>').text('Target Class QID (e.g. Q5 for humans) [Optional if schema is linked to class via P12861]: ').css({'display': 'block', 'font-weight': 'bold', 'margin-bottom': '6px'}));
        let $classInput = $('<input>')
            .attr('type', 'text')
            .attr('placeholder', 'Enter Class QID (e.g. Q5)')
            .addClass('cradle-input')
            .css({'max-width': '300px'});
        $classInputRow.append($classInput);
        $selectorDiv.append($classInputRow);

        // Input 3: Report Mode (Sample vs Deep)
        let $modeRow = $('<div>').css({'margin-bottom': '16px'});
        $modeRow.append($('<label>').text('Report Type: ').css({'display': 'block', 'font-weight': 'bold', 'margin-bottom': '6px'}));
        let $modeSelect = $('<select>').addClass('cradle-input').css({'max-width': '350px'});
        $modeSelect.append($('<option>').val('sample').text('Sample (Checks formatting & cardinality)'));
        $modeSelect.append($('<option>').val('deep').text('Deep Database Coverage (SPARQL - Non-invasive)'));
        $modeRow.append($modeSelect);
        $selectorDiv.append($modeRow);

        // Sample Specific Options Panel
        let $sampleOptions = $('<div>');

        // Input 4: Sample Size
        let $sizeRow = $('<div>').css({'margin-bottom': '16px'});
        $sizeRow.append($('<label>').text('SPARQL Sample Size (default 20, max 100): ').css({'display': 'block', 'font-weight': 'bold', 'margin-bottom': '6px'}));
        let $sizeInput = $('<input>')
            .attr('type', 'number')
            .attr('min', '5')
            .attr('max', '100')
        ...
        $sampleOptions.append($sizeRow);

        // Input 5: Manual QID list
        let $manualRow = $('<div>').css({'margin-bottom': '20px'});
        $manualRow.append($('<label>').text('OR Paste Manual QIDs list (comma-separated, e.g. Q42, Q100, Q80): ').css({'display': 'block', 'font-weight': 'bold', 'margin-bottom': '6px'}));
        let $manualInput = $('<textarea>')
            .attr('placeholder', 'Q42, Q100, Q80')
            .addClass('cradle-input')
            .css({'height': '60px', 'width': '100%', 'box-sizing': 'border-box'});
        $manualRow.append($manualInput);
        $sampleOptions.append($manualRow);

        $selectorDiv.append($sampleOptions);

        // Toggle sample options based on report mode
        $modeSelect.on('change', function() {
            if ($modeSelect.val() === 'deep') {
                $sampleOptions.hide();
            } else {
                $sampleOptions.show();
            }
        ...
        // Run Button
        let $btn = $('<button>').addClass('cradle-report-btn').text('Generate Report');
        $selectorDiv.append($btn);
        $container.append($selectorDiv);

        // Results Container
        let $resultsDiv = $('<div>').attr('id', 'cradle-report-results');
        ...
        $btn.on('click', function() {
            ...
            let classQid = $classInput.val().trim().toUpperCase();
            let sampleSize = parseInt($sizeInput.val()) || 20;
            let manualQidsText = $manualInput.val().trim();
            let reportMode = $modeSelect.val();

            if (!schemaId) {
                alert('Please enter an EntitySchema ID (e.g. E10)');
                return;
            }

            $resultsDiv.empty().append(
                $('<div>').addClass('cradle-report-card').append(
                    $('<h4>').text('Generating Report...'),
                    $('<div>').addClass('cradle-report-progress').append(
                        $('<div>').addClass('cradle-report-progress-bar').attr('id', 'cradle-report-bar')
                    ),
                    $('<p>').attr('id', 'cradle-report-status').text('Initializing schema parser...')
                )
            );

            // Fetch and parse schema properties
            loadSchemaProperties(schemaId).then(schemaProps => {
                if (reportMode === 'deep') {
                    // Deep report mode
                    $('#cradle-report-status').text('Checking target class for deep database report...');
                    $('#cradle-report-bar').css('width', '30%');

                    let targetClassPromise = classQid ? Promise.resolve(classQid) : getClassForSchema(schemaId);
                    
                    return targetClassPromise.then(targetClass => {
                        if (!targetClass) {
                            throw new Error(`Could not automatically find the class linked to schema ${schemaId}. Please enter the Class QID manually to generate a Deep Database Report.`);
                        }

                        $('#cradle-report-status').text(`Querying Wikidata database for class ${targetClass}...`);
                        $('#cradle-report-bar').css('width', '60%');

                        return runDeepSparqlReport(targetClass, schemaProps).then(deepRes => {
                            $('#cradle-report-bar').css('width', '100%');
                            displayDeepReportResults($resultsDiv, schemaId, targetClass, deepRes.total, schemaProps, deepRes.propCounts);
                        });
                    });
                } else {
                    // Sample report mode (existing logic)
                    $('#cradle-report-status').text('Obtaining item sample list...');
                    $('#cradle-report-bar').css('width', '20%');

                    let getQidsPromise;
                    if (manualQidsText) {
                        let qids = manualQidsText.split(/[\s,;|]+/).map(q => q.trim().toUpperCase()).filter(q => /^Q\d+$/.test(q));
                        getQidsPromise = Promise.resolve(qids);
                    } else if (classQid) {
                        getQidsPromise = getSampleQidsForClass(classQid, sampleSize);
                    } else {
                        getQidsPromise = getClassForSchema(schemaId).then(guessedClass => {
                            if (guessedClass) {
                                $('#cradle-report-status').text(`Found class ${guessedClass} linked to schema. Fetching sample...`);
                                return getSampleQidsForClass(guessedClass, sampleSize);
                            } else {
                                throw new Error(`No class linked to schema ${schemaId} via P12861. Please enter the Class QID manually.`);
                            }
                        });
                    }

                    return getQidsPromise.then(qids => {
                        if (qids.length === 0) {
                            throw new Error('No items found to validate.');
                        }
                        
                        $('#cradle-report-status').text(`Fetching data for ${qids.length} items...`);
                        $('#cradle-report-bar').css('width', '50%');

                        return fetchItemsData(qids).then(entities => {
                            return { qids: qids, entities: entities, schemaProps: schemaProps };
                        });
                    }).then(({ qids, entities, schemaProps }) => {
                        $('#cradle-report-status').text('Running validation checks...');
                        $('#cradle-report-bar').css('width', '80%');

                        let totalItems = qids.length;
                        let validCount = 0;
                        let totalErrors = 0;
                        let totalWarnings = 0;

                        let propertyStats = {}; // pid -> { present: 0, required: bool, label: string }
                        Object.keys(schemaProps).forEach(pid => {
                            let meta = propertyMetadata[pid] || { label: pid };
                            propertyStats[pid] = {
                                present: 0,
                                required: (schemaProps[pid].min >= 1),
                                label: meta.label || pid
                            };
                        });

                        let errorLog = []; // { qid, label, pid, error }
                        let itemScores = []; // { qid, label, score }

                        qids.forEach(qid => {
                            let entity = entities[qid];
                            if (!entity) return;
                            let label = (entity.labels && entity.labels[mw.config.get('wgUserLanguage') || 'en'] && entity.labels[mw.config.get('wgUserLanguage') || 'en'].value) || qid;
                            
                            let res = validateItemData(entity, schemaProps);
                            
                            // Conformance calculation
                            let allMandatoryPassed = (res.stats.presentRequired === res.stats.totalRequired);
                            if (allMandatoryPassed && res.errors.length === 0) {
                                validCount++;
                            }
                            ...
                            // Accumulate property presence
                            Object.keys(res.propStatus).forEach(pid => {
                                if (res.propStatus[pid].isPresent && res.propStatus[pid].isValid) {
                                    if (propertyStats[pid]) propertyStats[pid].present++;
                                }
                                if (res.propStatus[pid].errors.length > 0) {
                                    res.propStatus[pid].errors.forEach(errText => {
                                        errorLog.push({
                                            qid: qid,
                                            label: label,
                                            pid: pid,
                                            propLabel: propertyStats[pid] ? propertyStats[pid].label : pid,
                                            error: errText
                                        });
                                    });
                                }
                            ...
                            let totalProps = res.stats.totalRequired + res.stats.totalOptional;
                            let score = totalProps > 0 ? Math.round(((res.stats.presentRequired + res.stats.presentOptional) / totalProps) * 100) : 0;
                            itemScores.push({ qid: qid, label: label, score: score });
                        });

                        $('#cradle-report-bar').css('width', '100%');
                        displayReportResults($resultsDiv, schemaId, totalItems, validCount, totalErrors, totalWarnings, propertyStats, errorLog, itemScores);
                    });
                }
            }).catch(err => {
                $resultsDiv.empty().append(
                    $('<div>').addClass('cradle-report-card').css({'border-left': '4px solid var(--color-destructive, #d33)'}).append(
                        $('<h4>').css('color', 'var(--color-destructive, #d33)').text('Error Generating Report'),
    ...
    /**
     * Loads schema definition and metadata for report page.
    function loadSchemaProperties(schemaId) {
        let cleanId = schemaId.trim().toUpperCase();
        let schemaTitle = `EntitySchema:${cleanId}`;
        ...
        return api.get({
            ...
            if (pageId === "-1") {
                throw new Error(`Schema ${cleanId} not found in Wikidata.`);
            }
            
            let slot = pages[pageId].revisions[0].slots.main;
            let shexText = slot['*'];
            let parsed = parseShEx(shexText);
            
            if (Object.keys(parsed).length === 0) {
                throw new Error(`Could not parse any properties from schema ${cleanId}.`);
            }
            
            let propIds = Object.keys(parsed);
            
            // Load property metadata
            return loadPropertiesMetadata(propIds).then(meta => {
    ...
    /**
     * Gets sample items from SPARQL for a specific class.
     */
    function getSampleQidsForClass(classQid, limit) {
        let sparql = `SELECT ?item WHERE { ?item wdt:P31 wd:${classQid} . } LIMIT ${limit}`;
        return querySparql(sparql);
    }

    /**
     * Renders report output in DOM.
     */
    function displayReportResults($div, schemaId, total, valid, errors, warnings, propStats, errorLog, itemScores) {
        $div.empty();

        let conformanceRate = total > 0 ? Math.round((valid / total) * 100) : 0;

        let $summaryCard = $('<div>').addClass('cradle-report-card');
        $summaryCard.append($('<h3>').text(`Validation Report for Schema ${schemaId}`));
        
        // Stats grid
        let $grid = $('<div>').addClass('cradle-report-grid');
        $grid.append(
            $('<div>').addClass('cradle-report-stat-card').append(
                $('<div>').addClass('cradle-report-stat-num').text(total),
                $('<div>').text('Items Sampled')
            ),
            $('<div>').addClass('cradle-report-stat-card').append(
                $('<div>').addClass('cradle-report-stat-num').css('color', 'var(--color-success, #00af89)').text(`${conformanceRate}%`),
                $('<div>').text('Fully Conforming')
            ),
            $('<div>').addClass('cradle-report-stat-card').append(
                $('<div>').addClass('cradle-report-stat-num').css('color', 'var(--color-destructive, #d33)').text(errors),
                $('<div>').text('Total Errors')
            ),
            $('<div>').addClass('cradle-report-stat-card').append(
                $('<div>').addClass('cradle-report-stat-num').css('color', 'var(--color-warning, #fc3)').text(warnings),
                $('<div>').text('Warnings')
            )
        );
        $summaryCard.append($grid);
        $div.append($summaryCard);

        // Missing/Present Properties Card
        let $propsCard = $('<div>').addClass('cradle-report-card');
        $propsCard.append($('<h4>').text('Property Coverage in Sample'));
        let $propsTable = $('<table>').addClass('cradle-report-table');
        $propsTable.append(
            $('<thead>').append(
                $('<tr>').append(
                    $('<th>').text('Property'),
                    $('<th>').text('Type'),
                    $('<th>').text('Items Present'),
                    $('<th>').text('Coverage %')
                )
            )
        );
        let $propsBody = $('<tbody>');
        Object.keys(propStats).forEach(pid => {
            let stat = propStats[pid];
            let coverage = total > 0 ? Math.round((stat.present / total) * 100) : 0;
            $propsBody.append(
                $('<tr>').append(
                    $('<td>').append($('<a>').attr('href', `/wiki/Property:${pid}`).attr('target', '_blank').text(`${stat.label} (${pid})`)),
                    $('<td>').text(stat.required ? 'Required' : 'Optional'),
                    $('<td>').text(`${stat.present} of ${total}`),
                    $('<td>').css('font-weight', 'bold').text(`${coverage}%`)
                )
            );
        });
        $propsTable.append($propsBody);
        $propsCard.append($propsTable);
        $div.append($propsCard);

        // Errors Card (if any)
        let $errCard = $('<div>').addClass('cradle-report-card');
        $errCard.append($('<h4>').text('Validation Errors Found'));
        if (errorLog.length === 0) {
            $errCard.append($('<p>').text('✅ No invalid values or cardinality errors detected in the sample!'));
        } else {
            let $errTable = $('<table>').addClass('cradle-report-table');
            $errTable.append(
                $('<thead>').append(
                    $('<tr>').append(
                        $('<th>').text('Item'),
                        $('<th>').text('Property'),
                        $('<th>').text('Error Description')
                    )
                )
            );
            let $errBody = $('<tbody>');
            errorLog.forEach(row => {
                $errBody.append(
                    $('<tr>').append(
                        $('<td>').append($('<a>').attr('href', `/wiki/${row.qid}`).attr('target', '_blank').text(`${row.label} (${row.qid})`)),
                        $('<td>').text(`${row.propLabel} (${row.pid})`),
                        $('<td>').css('color', 'var(--color-destructive, #d33)').text(row.error)
                    )
                );
            });
            $errTable.append($errBody);
            $errCard.append($errTable);
        }
        $div.append($errCard);

        // Item health scores Card
        let $itemsCard = $('<div>').addClass('cradle-report-card');
        $itemsCard.append($('<h4>').text('Validated Items Overview'));
        let $itemsTable = $('<table>').addClass('cradle-report-table');
        $itemsTable.append(
            $('<thead>').append(
                $('<tr>').append(
                    $('<th>').text('Item QID'),
                    $('<th>').text('Label'),
                    $('<th>').text('Completion Score')
                )
            )
        );
        let $itemsBody = $('<tbody>');
        itemScores.forEach(row => {
            $itemsBody.append(
                $('<tr>').append(
                    $('<td>').append($('<a>').attr('href', `/wiki/${row.qid}`).attr('target', '_blank').text(row.qid)),
                    $('<td>').text(row.label),
                    $('<td>').text(`${row.score}%`)
                )
            );
        ...
        $itemsCard.append($itemsTable);
        $div.append($itemsCard);
    }
        ...
        return $.ajax({
            ...
            if (res && res.results && res.results.bindings) {
                return res.results.bindings.map(b => {
                    let val = b.item.value;
                    let match = val.match(/(Q\d+)$/);
    ...
    /**
     * Try to get target class from EntitySchema via P12861 backlinks.
     */
    function getClassForSchema(schemaId) {
        let sparql = `SELECT ?class WHERE { ?class wdt:P12861 wd:${schemaId.toUpperCase()} . } LIMIT 1`;
        let endpoint = 'https://query.wikidata.org/sparql';
        return $.ajax({
            url: endpoint,
            data: { query: sparql },
            ...
            if (res && res.results && res.results.bindings && res.results.bindings[0]) {
                let val = res.results.bindings[0].class.value;
                let match = val.match(/(Q\d+)$/);
                return match ? match[1] : null;
            }
            return null;
    ...
    /**
     * Batch fetch item entity data.
     */
    function fetchItemsData(qids) {
        let api = new mw.Api();
        let chunks = [];
        ...
        return Promise.all(promises).then(results => {
            ...
            results.forEach(res => {
                if (res && res.entities) {
                    Object.assign(entities, res.entities);
                }
            });
            return entities;
        });
    ...
    /**
     * Pure function to validate a Wikidata item data object against schema rules.
     */
    function validateItemData(entity, schemaProps) {
        ...
        Object.keys(schemaProps).forEach(pid => {
            let propDef = schemaProps[pid];
            let min = propDef.min !== undefined ? propDef.min : 0;
            let max = propDef.max !== undefined ? propDef.max : '*';
            let isRequired = (min >= 1);

            let entityClaims = claims[pid] || [];
            let activeClaims = [];
            entityClaims.forEach(c => {
                if (c.mainsnak && c.mainsnak.snaktype === 'value') {
                    let dv = c.mainsnak.datavalue;
                    if (dv && dv.value) {
                        activeClaims.push(dv.value);
                    }
                }
            });
            ...
            if (activeClaims.length < min) {
                propErrors.push(mw.msg('cradle-val-err-min', min));
            }
            if (max !== '*' && activeClaims.length > max) {
                propErrors.push(mw.msg('cradle-val-err-max', max));
            }

            activeClaims.forEach(val => {
                let datatype = propDef.datatype || 'string';
                if (datatype === 'wikibase-item') {
                    let qid = typeof val === 'object' ? val.id : val;
                    if (typeof qid === 'string' && !/^[qQ]\d+$/.test(qid.trim())) {
                        propErrors.push(mw.msg('cradle-val-err-qid', qid));
                    }
                } else if (datatype === 'quantity') {
                    let num = typeof val === 'object' ? val.amount : val;
                    if (typeof num === 'string' && isNaN(Number(num.trim()))) {
                        propErrors.push(mw.msg('cradle-val-err-num', num));
                    }
                }
            });

            let isValid = (propErrors.length === 0);

            if (isRequired) {
                stats.totalRequired++;
                if (isValid && isPresent) {
                    stats.presentRequired++;
                } else {
                    errors.push({ pid: pid, errors: propErrors });
                }
            } else {
                stats.totalOptional++;
                if (isValid && isPresent) {
                    stats.presentOptional++;
                } else if (!isPresent) {
                    warnings.push({ pid: pid, msg: mw.msg('cradle-val-optional-missing') });
                } else {
                    errors.push({ pid: pid, errors: propErrors });
                }
            }
    ...
    function liveUpdateValidation() {
        ...
        Object.keys(val.properties).forEach(pid => {
            ...
            $card.find('.cradle-card-validation-badge').text(propVal.badgeText);
    ...
    function renderForm() {
        ...
        $headerPanel.append($('<button>').addClass('cradle-btn-secondary').text(mw.msg('cradle-change-form')).on('click', changeActiveForm));
    ...
    /**
     * Executes server-side deep coverage SPARQL queries.
     */
    function runDeepSparqlReport(classQid, schemaProps) {
        let pids = Object.keys(schemaProps);
        if (pids.length === 0) return Promise.resolve({ total: 0, propCounts: {} });

        let valuesBlock = pids.map(pid => `wd:${pid}`).join(' ');
        
        let totalSparql = `SELECT (COUNT(?item) AS ?total) WHERE { ?item wdt:P31 wd:${classQid} . }`;
        let coverageSparql = `
            SELECT ?prop (COUNT(DISTINCT ?item) AS ?count) WHERE {
                VALUES ?prop { ${valuesBlock} }
                ?item wdt:P31 wd:${classQid} .
                ?item ?prop ?val .
            } GROUP BY ?prop
        `;

        let totalPromise = querySparqlRaw(totalSparql);
        let coveragePromise = querySparqlRaw(coverageSparql);

        return Promise.all([totalPromise, coveragePromise]).then(results => {
            let totalCount = 0;
            if (results[0] && results[0][0] && results[0][0].total) {
                totalCount = parseInt(results[0][0].total.value) || 0;
            }

            let propCounts = {};
            if (results[1]) {
                results[1].forEach(row => {
                    let propUri = row.prop.value;
                    let match = propUri.match(/(P\d+)$/);
                    if (match) {
                        let pid = match[1];
                        propCounts[pid] = parseInt(row.count.value) || 0;
                    }
                });
            }
    ...
    /**
     * Renders deep coverage SPARQL report results.
     */
    function displayDeepReportResults($div, schemaId, classQid, total, schemaProps, propCounts) {
        $div.empty();

        let $summaryCard = $('<div>').addClass('cradle-report-card');
        $summaryCard.append($('<h3>').text(`Deep Database Quality Report (SPARQL)`));
        $summaryCard.append($('<p>').html(`Analyzing all items of class <b><a href="/wiki/${classQid}" target="_blank">${classQid}</a></b> in the entire Wikidata database against Schema <b>${schemaId}</b>.`));
        
        // Total Items Badge
        $summaryCard.append(
            $('<div>').addClass('cradle-report-grid').append(
                $('<div>').addClass('cradle-report-stat-card').append(
                    $('<div>').addClass('cradle-report-stat-num').text(total.toLocaleString()),
                    $('<div>').text('Total Items in Database')
                )
            )
        );
        $div.append($summaryCard);
        ...
        // Coverage Table Card
        let $propsCard = $('<div>').addClass('cradle-report-card');
        $propsCard.append($('<h4>').text('Database-Wide Property Coverage'));
        let $propsTable = $('<table>').addClass('cradle-report-table');
        $propsTable.append(
            $('<thead>').append(
                $('<tr>').append(
        ...
        Object.keys(schemaProps).forEach(pid => {
            let propDef = schemaProps[pid];
            let meta = propertyMetadata[pid] || { label: pid };
            let count = propCounts[pid] || 0;
            let coverage = total > 0 ? Math.round((count / total) * 100) : 0;
            
            $propsBody.append(
                $('<tr>').append(
                    $('<td>').append($('<a>').attr('href', `/wiki/Property:${pid}`).attr('target', '_blank').text(`${meta.label || pid} (${pid})`)),
                    $('<td>').text(propDef.min >= 1 ? 'Required' : 'Optional'),
                    $('<td>').text(`${count.toLocaleString()} of ${total.toLocaleString()}`),
                    $('<td>').css('font-weight', 'bold').text(`${coverage}%`)
                )
            );
        });
        ...
        $propsCard.append(
            $('<p>')
                .css({'font-size': '0.8rem', 'color': 'var(--color-subtle, #54595d)', 'margin-top': '16px', 'font-style': 'italic'})
                .text('Note: Deep reports check global coverage across all items in Wikidata via the Wikidata Query Service (SPARQL). Individual formatting/syntax errors are not listed to prevent performance bottlenecks.')
        );
    ...
    /**
     * Query SPARQL and return raw bindings array.
     */
    function querySparqlRaw(sparql) {
        let endpoint = 'https://query.wikidata.org/sparql';
        return $.ajax({
            url: endpoint,
            data: { query: sparql },
            ...
            return (res && res.results && res.results.bindings) ? res.results.bindings : [];
        });
    }
